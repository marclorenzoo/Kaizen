...
import { Habit, UpdateHabitDto } from '../../models/habits.model';
...
@Component({
    ...
    templateUrl: './edit-habit-modal.html',
    styleUrl: './edit-habit-modal.scss',
})
export class EditHabitModal {
    ...
    constructor(
        ...
        this.habitForm = this.fb.group({
            ...
            icon: ['person', [Validators.required]],
    ...
    closeModal(): void {
        const modal = document.getElementById('edit_habit_modal') as HTMLDialogElement;
        modal?.close();
    }

    showModal(habit: Habit): void {
        this.currentHabit = habit;

        // Inicializar el formulario con los valores del hábito
        this.selectedIcon.set(habit.icono);
        this.selectedColor.set(habit.color);
        this.selectedFrequency.set(habit.frecuencia);
        this.customColor.set(habit.color);
    ...
    async onSubmit(): Promise<void> {
        if (this.habitForm.invalid || !this.currentHabit) {
        ...
        const result = await this.habitService.updateHabit(this.currentHabit.id, updateDto);

        if (result) {
            console.log('Hábito actualizado exitosamente:', result);
    ...
    async onDelete(): Promise<void> {
        ...
        const result = await this.habitService.deleteHabit(this.currentHabit.id);
        ...
        if (result) {
            ...
            console.error('Error al eliminar el hábito');
        }
    }
}
